...
import { AppState, View, Text, Alert, Image, Dimensions, Button, BackHandler, ActivityIndicator,  Picker, TextInput, StyleSheet, ScrollView, ImageBackground, StatusBar, TouchableOpacity, AsyncStorage } from 'react-native';
...
export class DataConfirm extends Component {
  constructor(props) {
    super();
  ...
  async openLink() {
    try {
      const url = `${this.state.server_url}/mobile/pay_airtime_online/${this.state.myParams.paymentCode}/${this.state.myParams.amount}/${this.state.myParams.phone}/${this.state.customer_id}/Data`
      if (await InAppBrowser.isAvailable()) {
        const result = await InAppBrowser.open(url, {
          // iOS Properties
  ...
  payWithWalletExec(){
    ...
    fetch(`${SERVER_URL}/mobile/pay_data_wallet`, {
      method: 'POST',
      headers: {
    ...
    return (
      <View style = {styles.body}>
        <LinearGradient start={{x: 0, y: 0}} end={{x: 1, y: 0}}  colors={['#e2aa2e', '#a75502']} style={styles.headerView}>
          ...
          <Text style = {styles.headerText}>Data Topup</Text>
          <TouchableOpacity style = {styles.backView}  onPress={() => this.props.navigation.navigate('Data')}>
          ...
          <View style={styles.container}>
            ...
            */}
            <TextInput
                        ...
                        />
              {/*
            <TouchableOpacity style = {styles.cardView}   onPress={() => this.setState({payWithCard: true,payWithWallet: false})} >
            ...
            */}
            <TouchableOpacity style = {styles.cardView}  onPress={() => this.setState({payWithWallet: true, payWithCard: false})}>
